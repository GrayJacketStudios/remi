...
export class LemmyClient extends FediverseClient {
  ...
  async login(username: string, password: string): Promise<{ token: string }> {
    const { data } = await axios.post(`${this.baseurl}/api/v3/user/login`, {
  ...
  async getPosts({ page = 0, limit = 10 } = {}): Promise<Post[]> {
    const data = await this.fetchJson<any>(`/api/v3/post/list?type_=All&sort=Active&page=${page}&limit=${limit}`);
